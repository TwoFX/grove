...
public structure AssociationLayer (kind : DataKind) (β : Type) where
  layerIdentifier : β
  layerValue : kind.Key

public structure Association (kind : DataKind) (β : Type) where
  id : String
  title : String
...
public structure Table (rowKind columnKind cellKind : DataKind) {δ : Type} (layerIdentifiers : List δ) : Type where
  id : String
  title : String
  description : String := ""
  /--
  ...
  allowedLayerCombinations : Option (Array (Array δ)) := none
